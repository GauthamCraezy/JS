...
public class sqlcontroller extends SQLiteOpenHelper 
{
	...
	{ String query; query = "CREATE TABLE Students ( StudentId INTEGER PRIMARY KEY, StudentName TEXT)";
	...
	{ String query; query = "DROP TABLE IF EXISTS Students"; 
	database.execSQL(query);
	onCreate(database); 
	}
	...
	//Log.d(LOGCAT,updateQuery); //database.rawQuery(updateQuery, null); //return database.update("words", values, "txtWord = ?", new String[] { word }); 
	} 
	...
	{ 
		...
		if (cursor.moveToFirst())
		{
			...
			map.put("StudentId", cursor.getString(0)); map.put("StudentName", cursor.getString(1));
			wordList.add(map);
			}
